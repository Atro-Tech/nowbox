<script lang="ts">
	...
	let mode = $state<'demo' | 'simple' | 'customize'>('demo');
	let host = $state('');
	let agent = $state('');
	let client = $state('');
	...
	const clients = [
		...
		{ id: 'app', icon: `<svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="2" y="3" width="10" height="8" rx="1"/><path d="M2 5h10"/></svg>` },
